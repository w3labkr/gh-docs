...
function sidebarHandler() {
  ...
  anchors.forEach(function (anchor) {
    ...
    anchor.addEventListener('click', function (e) {
      ...
      !!this.hash && setTertiary(this);
      !this.hash && setIframe(this);
...
function setTertiary(target) {
  var hashNode = document.querySelector(target.hash);
  var siblings = [].slice.call(hashNode.parentNode.children).filter(function (sibling) {
    return sibling !== hashNode;
  });
  ...
  var firstHashNode = document.querySelector([target.hash, 'a'].join(' '));
  firstHashNode.dispatchEvent(new Event('click'));
}
